  ...
  head: () => ({
    meta: [
      { title: "CompuRepuestos | Tecnología y repuestos de motos" },
      {
        name: "description",
        content:
          "Tecnología, artículos de oficina y repuestos de motos con entrega local en Venezuela.",
      },
...
const products = [
  {
    name: "Laptop Core i5 15.6 pulgadas",
    category: "Tecnología",
    price: 529,
    image: laptopImage,
    tag: "Oferta",
  ...
  {
    name: "Amortiguadores Racing 320 mm",
    category: "Motores",
    price: 89,
    image: shocksImage,
    tag: "Nuevo",
  ...
  {
    name: "Kit de clutch reforzado 200 cc",
    ...
    setCart((lines) => {
      ...
      if (existing) {
        return lines.map((line) =>
          line.name === product.name ? { ...line, quantity: line.quantity + 1 } : line,
        );
      }
      return [
        ...lines,
        { name: product.name, category: product.category, price: product.price, quantity: 1 },
      ];
    });
  ...
  return (
    <div className="min-h-screen bg-background text-foreground">
          ...
          <div className="ml-auto flex items-center gap-1 sm:gap-2">
            <Link to={user ? "/cuenta" : "/auth"} className="flex items-center">
              <Button
                variant="ghost"
                size="small"
                aria-label={user ? "Mi cuenta" : "Ingresar o registrarme"}
              >
            ...
            >
              ...
              {cartCount > 0 && (
                <span className="absolute right-0 top-0 grid size-5 place-items-center rounded-full bg-destructive text-[10px] font-bold text-destructive-foreground">
                  {cartCount}
                </span>
              )}
            </Button>
        ...
        <div className="px-4 pb-3 md:hidden">
          <label className="relative block">
            ...
            <input
              ...
              className="h-10 w-full rounded-sm border border-input bg-muted/60 pl-3 pr-10 text-sm outline-none focus:border-primary"
            />
      ...
      <main>
        <section className="relative grid min-h-[430px] grid-cols-1 overflow-hidden lg:grid-cols-2">
          <article className="group relative min-h-[330px] overflow-hidden bg-muted">
            <img
              src={techBanner}
              alt="Equipos de computación y artículos de oficina"
              width={1200}
              height={656}
            ...
            <div className="relative z-10 flex h-full min-h-[330px] max-w-xl flex-col items-start justify-end p-8 sm:p-12">
              ...
              <h1 className="max-w-sm font-display text-4xl leading-[1.05] text-primary sm:text-5xl">
                Todo para avanzar
              </h1>
              <p className="mt-3 max-w-sm text-base text-foreground/75">
                Computación, impresión y suministros para hacer más cada día.
              </p>
              <Button className="mt-6" onClick={() => setCategory("Tecnología")}>
                Ver tecnología <ChevronRight className="size-4" />
              </Button>
            </div>
          </article>
          ...
          <article className="group relative min-h-[330px] overflow-hidden bg-foreground">
            <img
              src={motoBanner}
              alt="Motor y repuestos para motocicletas"
              width={1200}
              height={656}
              loading="lazy"
              className="absolute inset-0 size-full object-cover transition duration-700 group-hover:scale-[1.02]"
            ...
            <div className="relative z-10 flex h-full min-h-[330px] max-w-xl flex-col items-start justify-end p-8 sm:p-12">
              <p className="mb-2 text-xs font-bold uppercase text-destructive-foreground/70">
                Repuestos de motos
              </p>
              <h2 className="max-w-md font-display text-4xl leading-[1.05] text-destructive-foreground sm:text-5xl">
                Potencia que responde
              </h2>
              <p className="mt-3 max-w-sm text-base text-destructive-foreground/75">
                Componentes confiables para mantener tu moto siempre en movimiento.
              </p>
              <Button variant="danger" className="mt-6" onClick={() => setCategory("Motores")}>
                Comprar repuestos <ChevronRight className="size-4" />
              </Button>
            </div>
          </article>
          <Button
            variant="outline"
            size="icon"
            className="absolute left-4 top-1/2 hidden -translate-y-1/2 rounded-full lg:inline-flex"
          ...
          <Button
            variant="outline"
            size="icon"
            className="absolute right-4 top-1/2 hidden -translate-y-1/2 rounded-full lg:inline-flex"
          ...
          {visibleProducts.length ? (
            <div className="grid grid-cols-2 gap-3 sm:gap-5 lg:grid-cols-4">
              {visibleProducts.map((product) => (
                <article
                  key={product.name}
                  className="group flex min-w-0 flex-col overflow-hidden rounded-md border border-border bg-card transition hover:-translate-y-1 hover:shadow-product"
                >
                  <div className="relative aspect-square overflow-hidden bg-muted">
                    ...
                    />
                    {product.tag && (
                      <span className="absolute left-3 top-3 rounded-sm bg-destructive px-2 py-1 text-[10px] font-bold uppercase text-destructive-foreground">
                        {product.tag}
                      </span>
                    )}
                  </div>
                  <div className="flex flex-1 flex-col p-3 sm:p-4">
                    ...
                    <h3 className="mt-1 min-h-10 text-sm font-semibold leading-tight text-card-foreground sm:text-base">
                      {product.name}
                    </h3>
        ...
        <section className="bg-primary text-primary-foreground">
          <div className="mx-auto grid max-w-7xl gap-px bg-primary-foreground/15 sm:grid-cols-3">
            {[
              ...
              return (
                <div key={String(title)} className="flex items-center gap-4 bg-primary px-6 py-7">
                  <ServiceIcon className="size-8 shrink-0 text-destructive" />
                  <div>
                    <h3 className="font-display text-base">{String(title)}</h3>
                    <p className="mt-1 text-sm text-primary-foreground/65">{String(text)}</p>
                  </div>
                </div>
              );
            })}
      ...
      <footer className="border-t border-border bg-background px-4 py-8">
        <div className="mx-auto flex max-w-7xl flex-col items-center justify-between gap-4 text-center sm:flex-row sm:text-left">
          ...
          <p className="text-sm text-muted-foreground">
            Tecnología, oficina y repuestos en un solo lugar.
          </p>
        </div>
      </footer>
      ...
      <a
        ...
        target="_blank"
        rel="noreferrer"
        className="fixed bottom-5 right-5 z-40 grid size-14 place-items-center rounded-full bg-whatsapp text-whatsapp-foreground shadow-floating transition hover:-translate-y-1"
      ...
      {cartOpen && (
        ...
        >
          <aside
            className="flex h-full w-full max-w-sm flex-col bg-background p-6 shadow-floating"
            onClick={(event) => event.stopPropagation()}
          >
            <div className="flex items-center justify-between">
              <h2 className="font-display text-xl text-primary">Tu carrito</h2>
              <Button variant="ghost" size="small" onClick={() => setCartOpen(false)}>
                Cerrar
              </Button>
            </div>
            {cart.length ? (
      ...
      {notice && (
        <div
          role="status"
          className="fixed bottom-5 left-1/2 z-50 -translate-x-1/2 rounded-sm bg-foreground px-4 py-3 text-sm font-semibold text-background shadow-floating"
        >
          {notice}
        </div>
      )}
